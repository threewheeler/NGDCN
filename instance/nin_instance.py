...
class NINInstance:
    def __init__(self, args):
        ...
        self.manager = tf.train.CheckpointManager(self.ckpt, f'./tf_ckpts_{self.args.model_args["model_prefix"]}',
                                                  max_to_keep=10)
    ...
    def load_data(self):
        self.dataset = getattr(dataset, self.args.dataset)(self.args)

    def load_model(self):
        self.model = getattr(model, self.args.model)(self.args).model

    # @tf.function
    def train_step(self, images, labels, is_test=False):
    ...
    def train_and_validate(self, epochs, is_test=False):
        ...
        for epoch in range(epochs):
            ...
            if is_test is True:
                train_ds_for_test = self.dataset.train_ds_for_test
            for (batch, (images, labels)) in enumerate(train_ds_for_test):
                self.train_step(images, labels, is_test)
                if is_test:
                    return
